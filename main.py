...
while True:
    try:
        ...
        if start_time < now < start_time + datetime.timedelta(minutes=2) or len(today_coin_list) == 0:
            # 우선 리스트를 비운다.
        ...
        if start_time + datetime.timedelta(minutes=2) < now < end_time - datetime.timedelta(minutes=5):
            # 코인별 정보 확인
            for i, e in enumerate(today_coin_list):
                ...
                if is_trade == 0:
                    ...
                    if current_price > target_price and current_price < predicted_close_price:
                        ...
                        print(f"\033[31mBuyout Reached\033[0m: \033[35m{market_code}\033[0m")
                        # 업비트 최소 거래 금액
                        if my_krw > MONEY:
                            # 매수 진행
